Ext.onReady(function() {
	...
	genGrid = new GenGrid({
				...
				actionurl : '.\/json\/productquery.action',
				autoHeight : true,
	...
	// genGrid.grid.on('rowdblclick', function(grid, rowIndex, e) {
	// var sele = genGrid.grid.getSelectionModel().getSelections();
	// var id = sele[0].get('id');
	// show(id);
	//
	// });

	Ext.get('proqueryadd').remove();
	Ext.get('proquerymodi').remove();
	Ext.get('proquerydel').remove();
	...
	var qpn = new Ext.Toolbar({
				...
				items : [{
							...
							text : '曾返修产品',
							handler : function() {
								genGrid.getData(1, 25, ' and  repairnum> 0 ');
							}
						}, {
							text : '本月生产',
							handler : function() {
	...
	var engcm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer({
						header : '序号',
						width : 30
					}),// 自动行号
			{
	...
	engcm.defaultSortable = true;

	var engdata;

	var engds = new Ext.data.Store({
				proxy : new Ext.data.MemoryProxy(engdata),
				reader : new Ext.data.JsonReader({
							root : 'list'
						}, [{
									name : 'id'
	...
	engds.load();
	var enggrid = new Ext.grid.GridPanel({
				ds : engds,
				cm : engcm,
				title : '产品安装',
				autoWidth : true,
	...
	var tabpanel = new Ext.TabPanel({
				autoHeight : true,
				autoWidth : true,
				items : [flowgrid, enggrid,repairgrid]
			});

	tabpanel.setActiveTab(flowgrid);
	...
	thistab.on('blur', function() {
				detailwin.hide();
			});

		// var qpn = new Ext.Panel({
		// width : Ext.get('maintab').getWidth() - 20,
		// height : 20,
		// renderTo : thistab,
