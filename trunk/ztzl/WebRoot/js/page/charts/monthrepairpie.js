...
Ext.onReady(function() {
	...
	var tbar = new Ext.Toolbar({
				...
				items : [{
							...
							handler : function() {
								...
								var condi = " and senddate between to_date('"
										+ start.value
										+ "','yyyy-mm') and to_date('"
										+ end.value + "','yyyy-mm') ";
								getData(condi);
	...
	function getData(condi) {
		Ext.Ajax.request({
					url : '.\/json\/monthrepairpie.action',
					params : {
						pageNo : 1,
						pageCount : 100,
						Condi : condi
	...
	function process(list) {
		var myChart = new FusionCharts(combo.getValue(), "monthtypepie", "600",
				"400");
		var xmlData = getHead('产品返修比例', '故障现象', '数量');;

		for (var i = 0; i < list.length; i++) {
			xmlData += getBody(listError(list[i][0]), list[i][1], randomColor());
		}
		xmlData += getTail();
