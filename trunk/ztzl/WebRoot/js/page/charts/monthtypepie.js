...
function getHead(caption, xa, ya) {
	return "<graph baseFontSize='12' caption='" + caption + "' xAisName='" + xa
			+ "' yAxisName='" + ya
...
Ext.onReady(function() {
	...
	var data = [['Charts/Area2D.swf', '二维区域图'], ['Charts/Bar2D.swf', '二维条图'],
			['Charts/Column2D.swf', '二维柱状图'], ['Charts/Column3D.swf', '三维柱状图'],
			['Charts/Doughnut2D.swf', '二维环图'],
			['Charts/Doughnut3D.swf', '三维环图'], ['Charts/Pie2D.swf', '二维饼图'],
			['Charts/Pie3D.swf', '三维饼图']];
	...
	combo.setValue('Charts/Pie3D.swf');
	...
	function listType(val) {
		...
		var tp = store1.getAt(store1.find('id', val));
		if (tp)
	...
	function process(list) {
		var myChart = new FusionCharts(combo.getValue(), "monthtypepie", "600",
				"400");
		var xmlData = getHead('产品类别比例', '类别', '数量');;

		for (var i = 0; i < list.length; i++) {
			xmlData += getBody(listType(list[i][0]), list[i][1], randomColor());
		}
		xmlData += getTail();
