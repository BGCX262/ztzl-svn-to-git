...
Main.prototype = {
				...
				tbar : [{
							...
							handler : function() {
								Ext.Msg
										.alert(
												'关于 Ext界面生成器',
												"作者 : Christophe Badoit (extjs a t tof2k d o t com)<br/>"
														+ '汉化 : <a target=\"_blank\" href=\"http://www.dojochina.com/\">www.dojochina.com</a>');
							}
						}]
			}, {
				region : 'west',
	...
	initTreePanel : function() {
		...
		tree.on('beforenodedrop', function(de) {
					if (!de.rawEvent.ctrlKey) {
						this.markUndo("移动了 " + de.dropNode.text);
						return true;
					}
					this.markUndo("复制了 " + de.dropNode.text);
		...
		var contextMenu = new Ext.menu.Menu({
			items : [{
						...
						text : '实时调整',
						tooltip : '实时调整当前元素.<br/>在当前元素是<b>绝对</b> 定位前提下，你也可以拖动此元素',
						iconCls : 'icon-resize',
						scope : this,
						handler : function(item) {
		...
		var contextMenu = new Ext.menu.Menu({
					items : [{
						...
						handler : function(item, e) {
							this.markUndo('删除属性<i>' + item.record.id + '</i>');
							var ds = grid.store;
	...
	updateUndoBtn : function() {
		if (this.undoHistory.length == 0) {
			Ext.ComponentMgr.get('FBUndoBtn').disable().setText('取消');
		} else {
			Ext.ComponentMgr.get('FBUndoBtn').enable().setText('<b>取消</b> : '
	...
	visualResize : function(node) {
		...
		if (node.parentNode && node.parentNode.elConfig
			...
			Ext.Msg
					.alert(
							"Error",
							"You won't be able to resize an element"
	...
	updateForm : function(force, node) {
		...
		if (time && updateTime) {
			Ext.ComponentMgr.get('FBRenderingTimeBtn').setText('生成时间 : <i>'
							+ time + '毫秒</i>');
		}
	},
	...
	canAppend : function(config, node) {
		...
		if (xtype
				&& ['panel', 'viewport', 'form', 'window', 'tabpanel',
						'toolbar', 'fieldset'].indexOf(xtype) == -1) {
				...
				if (!useHasOwn || o.hasOwnProperty(i)) {
					...
					switch (typeof v) {
						...
						default :
							...
							a.push(indentStr(indent), i, ":", v === null
											? "null"
											: this.encode(v, indent + 1));
							b = true;
					}
				}
			}
