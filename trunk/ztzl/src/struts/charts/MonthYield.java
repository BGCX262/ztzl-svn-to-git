...
public class MonthYield extends struts.BaseAction {
	...
	private static String queryString = " select to_char(prodate,'yyyy-mm') as mn ,count(*) as cn from Products where 1=1 ";
	private static String order = " group by to_char(prodate,'yyyy-mm')  ";
	...
	public String exe() {
		...
		if (getCondi() == null || getCondi().equals(""))
			setCondi(" and prodate between add_months(sysdate,-6) and sysdate ");
	...
	public static void setQueryString(String queryString) {
		MonthYield.queryString = queryString;
	}
