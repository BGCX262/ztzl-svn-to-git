...
import hibernate.pojo.Errorfor;
import hibernate.pojo.Errortype;

import java.util.List;

public class ErrorForMan extends BaseAction {
	...
	private static String queryString = " from Errorfor where 1=1 ";
	...
	public String addnew() {
		try {
			...
			if (getId().length() > 0) {
				errorfor = (Errorfor) super.findByID("Errorfor", Long
						.valueOf(str2int(getId())));
			} else
				errorfor = new Errorfor();

			errorfor.setErrotype(this.getErrotype());
			errorfor.setErrorview(this.getErrorview());
			errorfor.setErrorinfo(this.getErrorinfo());
			errorfor.setInuse(this.getInuse());

			if (getId().length() > 0)
				super.update(errorfor);
			else
				super.save(errorfor);
			return null;
		} catch (Exception e) {
