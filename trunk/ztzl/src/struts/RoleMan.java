...
import hibernate.pojo.Roles;

import java.util.List;

import common.MD5Sec;

public class RoleMan  extends BaseAction {
	...
	private static String queryString=" from Roles where 1=1 ";
	...
	public String addnew() {
		try {
			...
			if (getId().length() > 0) {
				roles = (Roles) super.findByID("Roles",
						Long.valueOf(str2int(getId())));
			} else
			{
				roles = new Roles();
				int i=super.getCount(" from Roles where 1=1 and rolename='"+getRolename() +"' ");
				if(i>0)
				{
					this.setRtntxt("用户名已存在!");
					return SUCCESS;
				}
			}
			...
			roles.setRolename(this.getRolename());
			roles.setRoleinfo(this.getRoleinfo());
			roles.setRolememo(this.getRolememo());
